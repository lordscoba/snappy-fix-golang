...
func Throttle() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		if isExemptIP(c.ClientIP(), serverConfig.ExemptFromThrottle) {
			...
			if httpError != nil {
				c.AbortWithStatusJSON(httpError.StatusCode, responses.BuildErrorResponse(httpError.StatusCode, "error", lmt.GetMessage(), httpError.Message, nil))
			} else {
				c.Next()
			}
		}
...
func Security() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Content Security Policy - Modified to allow inline scripts, styles, and data URIs for images.
		// NOTE: 'unsafe-inline' should be used with caution as it can slightly weaken security.
		// c.Writer.Header().Add("Content-Security-Policy", "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; img-src 'self' data:;")
